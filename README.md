...
#### ***present로 표시한 뷰 컨트롤러를 pop할 수 있나요?***
#### ***push로 표시한 뷰 컨트롤러를 dismiss할 수 있나요?***
...
- 기능을 처리할 객체를 델리케이트로 설정하고, 특정 이벤트가 발생할때 델리케이트에의해 위임된 본래의 객체로 전달해주는 역할을 한다.
- 델리게이트 객체에게 필요한 복잡한 동작을 위임하고 '나'에게는 특정 이벤트가 발생했을때 알려주는 방식으로 동작.
- 델리게이트 메소드를 사용해서 특정 이벤트가 발생했을때 알려준다.
- 델리게이트는 프로토콜을 통해 이미 약속되어있으므로, iOS에서 델리게이트 패턴을 사용하는 모든 객체는 델리게이트 메소드를 이용하려면 델리게이트 프로토콜을 구현해야한다.
- 이 때 프로토콜의 이름은 항상 해당 객체의 클래스명 + Delegate이다.
...
#### 최초 응답자
- UIWindow객체: 사용자 인터페이스 구조에서 사용자에 가장 가까이 있는 객체, 사용자로부터 발생하는 터치관련 이벤트를 내부 객체로 전달하는 역할을 담당한다. 이를위해 UIWindow는 이벤트가 발생했을때 우선적으로 응답할 객체를 가리키는 최초응답자라는 포인터를 가지고있다.
- 사용자가 화면에서 텍스트필드를 터치하면 윈도우는 최초 응답자 포인터를 해당 텍스트 필드로 옮겨줌.
